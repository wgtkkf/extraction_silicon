...
# read generic coordinates from ABINIT input (manually prepared)
...
# read generic coordinates from ABINIT output (outputs by ABINIT)
...
## added routine for VL & VR of a gap system
comments_input()
file_num_keyboard = input()
file_num = np.float64(file_num_keyboard) # gap distence [angs]
file_num = file_num*ucangs/ucbohr # gap distance [bohr]

# extraction start
for n in range(-1, 2, 1): # zloop (-1 -> 0 -> +1)
    # update gapKCL & gapKCR depending on n value (zloop)
    if n==-1:
        gapKCL = 0
        gapKCR = file_num # for VCR
    elif n==0:
        gapKCL = 0
        gapKCR = 0
    else: # n==+1
    ...
    # z direction
    s_coordinate1[2] = coordinate1[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate2[2] = coordinate2[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate3[2] = coordinate3[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate4[2] = coordinate4[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate5[2] = coordinate5[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate6[2] = coordinate6[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate7[2] = coordinate7[2] + (2*n)*lcsi_bohr + n*gapKCL
    s_coordinate8[2] = coordinate8[2] + (2*n)*lcsi_bohr + n*gapKCL
    # gap is here
    s_coordinate9[2] = coordinate9[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate10[2] = coordinate10[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate11[2] = coordinate11[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate12[2] = coordinate12[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate13[2] = coordinate13[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate14[2] = coordinate14[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate15[2] = coordinate15[2] + (2*n)*lcsi_bohr + n*gapKCR
    s_coordinate16[2] = coordinate16[2] + (2*n)*lcsi_bohr + n*gapKCR
    ...
    for m in range(0, dof): # x, y & z loop
        ...
        s_coordinate9[m] = coordinate9[m]
        s_coordinate10[m] = coordinate10[m]
        s_coordinate11[m] = coordinate11[m]
        s_coordinate12[m] = coordinate12[m]
        s_coordinate13[m] = coordinate13[m]
        s_coordinate14[m] = coordinate14[m]
        s_coordinate15[m] = coordinate15[m]
        s_coordinate16[m] = coordinate16[m]
